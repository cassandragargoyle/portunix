...
func loadDefaultConfig() (*InstallConfig, error) {
	// Since we removed the embedded install-packages.json, create a minimal config
	// The new registry-based system in LoadPackageRegistry handles individual package files
...
func GetLinuxDistribution() (string, string, error) {
	...
	} else if strings.Contains(distro, "arch") || strings.Contains(distro, "arch linux") {
...
func (config *InstallConfig) GeneratePresetListDescription() string {
	...
	for name, preset := range config.Presets {
		...
		for _, pkg := range preset.Packages {
			if strings.Contains(pkg.Name, "claude") || strings.Contains(pkg.Name, "gemini") || strings.Contains(pkg.Name, "ai-") {
				isAIPreset = true
				break
			}
		}
	...
	if len(aiPresets) > 0 {
		...
		for name, preset := range aiPresets {
	...
	if len(otherPresets) > 0 {
		result.WriteString("\nOther Presets:\n")
		for name, preset := range otherPresets {
			result.WriteString(fmt.Sprintf("  %-20s - %s\n", name, preset.Description))
		}
	}
...
func (config *InstallConfig) GenerateVariantListDescription() string {
	...
	for _, name := range packageNames {
		...
		if !exists && len(pkg.Platforms) > 0 {
			// Try linux as fallback for documentation
