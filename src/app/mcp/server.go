...
// StartStdioNewline starts the MCP server in stdio mode with proper newline-delimited JSON-RPC
func (s *Server) StartStdioNewline() error {
	...
	fmt.Fprintf(os.Stderr, "MCP server starting - stdio mode with newline-delimited JSON-RPC\n")
	fmt.Fprintf(os.Stderr, "Permission level: %s\n", s.Permissions)
	...
	for {
		...
		default:
			...
			var request MCPRequest
			if err := json.Unmarshal([]byte(line), &request); err != nil {
				fmt.Fprintf(os.Stderr, "Error parsing JSON-RPC: %v (line: %s)\n", err, line)
				continue
			}
			...
			// Marshal response to JSON (compact, no whitespace)
			responseJSON, err := json.Marshal(response)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error marshaling response: %v\n", err)
				continue
...
func (s *Server) StartTCP(port int) error {
	...
	go func() {
		...
		signal.Notify(sigChan, syscall.SIGINT, syscall.SIGTERM)
		<-sigChan
		fmt.Fprintf(os.Stderr, "Shutting down MCP TCP server...\n")
		cancel()
		listener.Close()
	}()

	fmt.Fprintf(os.Stderr, "MCP Server listening on TCP port %d\n", port)
	fmt.Fprintf(os.Stderr, "Permission level: %s\n", s.Permissions)
...
func (s *Server) StartUnix(socketPath string) error {
	...
	go func() {
		...
		signal.Notify(sigChan, syscall.SIGINT, syscall.SIGTERM)
		<-sigChan
		fmt.Fprintf(os.Stderr, "Shutting down MCP Unix socket server...\n")
		cancel()
		listener.Close()
		os.Remove(socketPath)
	}()

	fmt.Fprintf(os.Stderr, "MCP Server listening on Unix socket: %s\n", socketPath)
	fmt.Fprintf(os.Stderr, "Permission level: %s\n", s.Permissions)
...
// handleSocketConnection handles a Unix socket connection
...
// handleGenericConnection handles any net.Conn with newline-delimited JSON-RPC
func (s *Server) handleGenericConnection(conn net.Conn) {
	...
	for {
		...
		if err != nil {
			if netErr, ok := err.(net.Error); ok && netErr.Timeout() {
				...
				ping := MCPResponse{
					JSONRPC: "2.0",
					Result:  map[string]string{"type": "ping"},
				}
				if pingJSON, err := json.Marshal(ping); err == nil {
