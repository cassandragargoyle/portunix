...
// Package packageregistry provides shared package-registry types and
// loading logic (legacy v1 schema). It is shared between the main portunix
// binary (cmd/registry.go) and helper binaries that need to read the
// embedded or external assets/packages/ index.
//
// Related: ADR-026 (Shared Platform Utilities), Issue #186a.
...
// ConvertToLegacyConfig converts the registry to legacy InstallConfig format for backward compatibility
func (r *PackageRegistry) ConvertToLegacyConfig() (*installconfig.InstallConfig, error) {
	config := &installconfig.InstallConfig{
		Version:  "1.0",
		Packages: make(map[string]installconfig.PackageConfig),
		Presets:  make(map[string]installconfig.PresetConfig),
	}
...
func (r *PackageRegistry) convertPackageToLegacy(pkg *Package) (*installconfig.PackageConfig, error) {
	legacyPkg := &installconfig.PackageConfig{
		...
		Platforms:   make(map[string]installconfig.PlatformConfig),
	}

	// Find default variant - look for "default" or take first one
	defaultVariant := ""
	for _, platform := range pkg.Spec.Platforms {
		for variantName := range platform.Variants {
			if variantName == "default" || variantName == "stable" {
	...
	legacyPkg.DefaultVariant = defaultVariant

	// Convert platforms
	for platformName, platform := range pkg.Spec.Platforms {
		legacyPlatform := installconfig.PlatformConfig{
			Type:        platform.Type,
			Variants:    make(map[string]installconfig.VariantConfig),
		...
		if platform.Verification != nil {
			legacyPlatform.Verification = installconfig.VerificationConfig{
		...
		// Convert variants
		for variantName, variant := range platform.Variants {
			legacyVariant := installconfig.VariantConfig{
...
func (r *PackageRegistry) loadPackageFromEmbedded(filename string) error {
	packagePath := filepath.Join("assets/packages", filename)
